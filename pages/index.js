...
export async function getStaticProps() {
  ...
  // console.log("phoneme_str", phoneme_str);
  ...
  const countSyllables = (word) => {
    ...
    if (word.endsWith("'s") || word.endsWith("’s")) {
      ...
      console.log("phonemeStr", phonemeStr, "word", word);
      ...
      // # print(cmudict[word])
      const phonemes = phonemeStr.split(" ");
      console.log("phoneme", phonemes);

      for (const phoneme of phonemes) {
        console.log("phoneme", phoneme.slice(phoneme.length - 1));
        const lastChar = phoneme.slice(phoneme.length - 1);
        console.log("lastChar", lastChar, parseInt(lastChar, 10));
        if (Number.isInteger(parseInt(lastChar, 10))) {
          numSylls += 1;
        }
      }
  ...
  // console.log("countSyllables", countSyllables("people's"));
  ...
  const getComments = async () => {
    const results = await Promise.allSettled(
      data.results.map(async (result) => {
        ...
        const comments = data.results.comments.map((comment) =>
          // console.log(comment.commentBody);
          comment?.commentBody.split("\n").join(" ")
        );

        // const filtered = comments.filter((comment) => comment !== "\n");
        // console.log(comments.join(" ").split(" "));

        const dataResult = comments.join(" ").split(" ");
  ...
  const constructHaikuLine = (
    ...
    if (endPrevLine.length == 0) {
      line = "1";
      let { word, numSyls } = randomWord(corpus);
      // console.log(numSyls);
      ...
      lineSyls += numSyls;
      let wordChoices = wordAfterSingle(word, suffixMap1, lineSyls, targetSyls);
      while (wordChoices.length == 0) {
        // const prefix = random.choice(corpus)
        const prefix = corpus[Math.floor(Math.random() * corpus.length)];
        wordChoices = wordAfterSingle(prefix, suffixMap1, lineSyls, targetSyls);
      }
    ...
    while (true) {
      ...
      wordChoices = wordAfterSingle(prefix, suffixMap2, lineSyls, targetSyls);
  ...
  const wordAfterSingle = (prefix, suffixMap, currentSyls, targetSyls) => {
